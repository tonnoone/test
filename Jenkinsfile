        ...
        stage('Обновление тестового контура') {
            steps {
                timestamps {
                    ...
                    cmd("deployka loadrepo ${connectionString} \"${env:StoragePath}\" -storage-user ${env:Storage_Usr} -storage-pwd ${env:Storage_Psw} -v8version \"8.3.18.1334\" -db-user ci-bot-ci-bot -db-pwd 123 -uccode ${uccode}")
