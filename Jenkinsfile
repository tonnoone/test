...
pipeline {
    ...
    stages {
        stage('Обновление тестового контура') {
            steps {
                timestamps {
                    ...
                    cmd("deployka loadrepo ${connectionString} \"H:\" -storage-user ${env:Storage_Usr} -storage-pwd ${env:Storage_Psw} -v8version \"8.3.18.1520\" -db-user ci-bot -db-pwd 123 -uccode ${uccode}")
                    cmd("deployka dbupdate ${connectionString} -allow-warnings -uccode ${uccode} -db-user ci-bot -db-pwd 123 -v8version \"8.3.18.1520\"")
        ...
        stage('Проверка поведения') {
            steps {
                timestamps {
                    cmd("vrunner vanessa --vanessasettings ./tools/VBParams.json --workspace . --ibconnection ${connectionString} --db-user ci-bot --db-pwd 123 --v8version \"8.3.18.1520\"")
                }
            }
        }
    }
